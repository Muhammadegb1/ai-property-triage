# Prompt Engineering Log — LangGraph Tool Descriptions
...
**Change:** Added JSON output format with args structure.
...
**What improved:** Image analyser calls now work (image URLs are visible in the query).
**What still fails:** `query_similar_listings` never called — GPT doesn't know what value to pass as the description arg.
...
**Change:** Added explicit trigger conditions to the query_similar_listings description.
...
**Assessment:** No improvement. The tool description lists triggers but GPT still doesn't know what to pass as the description argument, so it skips the tool entirely.
...
**Change:** Added explicit arg format and Rules section to guide argument extraction.
...
**What improved:** Query 10 (listings + image) now passes — explicit Rules helped with combined queries.
**What still fails:** `query_similar_listings` fails for queries where the user asks a question (no explicit property description).
...
**Change:** Rewrote the Rules section with three targeted fixes:
...
| v5 | 10/10 | Added "even if no full listing description" + multi-tool rule |
